...

{
	if (_x call config_dropable)then
	{
		...
		["ALL",[_object,['','noscript.sqf',format['%1 call items_pickup',[_object, _x, _amount]],25,false,true,'LeanRight',format ['player distance _target < 5 && {!([_target,"Pick up (%3) %1 (E)","\crp_data\images\items\%2"]call tag_show)}',_name13,[_x]call config_image,_amount]]],"network_addAction",false,true]call network_MPExec;
	};
}Foreach _items;
